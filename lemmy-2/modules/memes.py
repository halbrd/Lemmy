...
class Memes(Module):
	...
	async def cmd_fusion(self, message, args, kwargs):
		...
		def interpret(input):
			if input == '?':
				input = random.choice(list(Memes.pokemon_codes.keys()))

			letters = set('abcdefghijklmnopqrstuvwxyz')
			return ''.join(map(lambda char: char if char in letters else '', input.lower()))

		first_code = Memes.pokemon_codes.get(interpret(args[0]))
		second_code = Memes.pokemon_codes.get(interpret(args[1]))
